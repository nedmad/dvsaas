  ...
  return (
    <section className="flex flex-col items-center w-full max-w-lg">
      ...
      <form
        ...
        className="flex gap-3 flex-col w-full max-w-md"
      >
        ...
        <button
          disabled={loading}
          className={`w-full flex justify-center bg-blue-600 rounded text-white font-bold py-2 px-2 ${loading && style.button}`}
        >
          {loading && <FiLoader className="animate-spin" size={28} />}
          {!loading && "Entrar"}
        </button>
      </form>
      <Link href={"/auth/resend"} className="text-blue-500 text-center mt-5">
        Esqueci minha senha! Clique aqui para recuperar senha.
      </Link>
    </section>
  );
}
